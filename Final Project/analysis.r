...
test_theories <- function(window, vocab_by, vocab_minimum, by_grade = F) {
  df <- paste('learningPotential', window, vocab_by, vocab_minimum, sep = '_')
  df <- read.xlsx(paste0(df,'.xlsx'))
...
test_theories(window = "2", vocab_by = "speakers", vocab_minimum = "3", by_grade = T)
